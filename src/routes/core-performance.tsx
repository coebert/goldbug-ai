...
function CorePerformancePage() {
  ...
  const chartRows = useMemo(() => mergeNormalisedSeries(data?.funds ?? []), [data]);
  const core = data?.funds[0];
  const available = data?.funds.filter((fund) => fund.metrics) ?? [];
  ...
  >
    <div className="space-y-5">
      ...
      : <>
        ...
        <section className="space-y-2" aria-labelledby="comparison-title"><div><h2 id="comparison-title" className="text-base font-semibold">Return and risk comparison</h2><p className="text-xs text-muted-foreground">Peers with unavailable history stay visible but are excluded from the chart and ranking.</p></div><div className="overflow-x-auto rounded-md border border-border bg-card"><Table><TableHeader><TableRow><TableHead>Fund</TableHead><TableHead className="text-right">Return</TableHead><TableHead className="text-right">Annualised</TableHead><TableHead className="text-right">Volatility</TableHead><TableHead className="text-right">Max drawdown</TableHead><TableHead className="text-right">vs VWRL</TableHead></TableRow></TableHeader><TableBody>{data.funds.map((fund) => <TableRow key={fund.symbol}><TableCell><div className="font-medium">{fund.symbol}</div><div className="max-w-56 text-xs text-muted-foreground">{fund.name}</div></TableCell><TableCell className="text-right tabular-nums">{percent(fund.metrics?.totalReturn ?? null)}</TableCell><TableCell className="text-right tabular-nums">{percent(fund.metrics?.annualisedReturn ?? null)}</TableCell><TableCell className="text-right tabular-nums">{riskPercent(fund.metrics?.annualisedVolatility ?? null)}</TableCell><TableCell className="text-right tabular-nums">{riskPercent(fund.metrics?.maxDrawdown ?? null)}</TableCell><TableCell className="text-right tabular-nums">{fund.metrics && core.metrics ? percent(fund.metrics.totalReturn - core.metrics.totalReturn) : "—"}</TableCell></TableRow>)}</TableBody></Table></div></section>

        <section className="space-y-2" aria-labelledby="allocation-title"><div><h2 id="allocation-title" className="text-base font-semibold">What the 50% target changes</h2><p className="text-xs text-muted-foreground">Half the account follows the fund; the other half is held flat. Trading profits, costs and rebalancing are excluded.</p></div><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">{data.funds.map((fund) => <Card key={fund.symbol}><CardHeader className="pb-2"><CardTitle className="text-sm">50% in {fund.symbol}</CardTitle></CardHeader><CardContent><div className="text-2xl font-semibold tabular-nums">{percent(fund.targetAllocationReturn)}</div><p className="mt-1 text-xs text-muted-foreground">account impact over {years} years</p></CardContent></Card>)}</div><div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-muted-foreground"><span>Current core: <strong className="text-foreground">{(data.currentPct * 100).toFixed(1)}%</strong></span><span>Target: <strong className="text-foreground">{(data.targetPct * 100).toFixed(0)}%</strong></span><span>Target band: <strong className="text-foreground">±{(data.bandPct * 100).toFixed(0)}%</strong></span><span>Current value: <strong className="text-foreground">{formatMoney(data.coreValueBase, "GBP")}</strong></span></div></section>
      </>}
    </div>
  </PageShell></>;
}
