...
export function formatShortSleeveBlock(args: {
  ...
  const lines = [
    ...
    `  Current: long ${(e.longPctNavSafe ?? e.grossPctNav - e.shortPctNav) * 100 >= 0 ? (((e.longValue / (nav || 1)) * 100).toFixed(0)) : "0"}% NAV, short ${(e.shortPctNav * 100).toFixed(0)}% NAV, net ${(e.netPctNav * 100).toFixed(0)}%, gross ${(e.grossPctNav * 100).toFixed(0)}%`,
