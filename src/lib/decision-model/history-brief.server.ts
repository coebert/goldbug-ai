...
export type HistoryBrief = {
  coverage: {
    ...
    /** Split of that cost: invoiced charges vs realised slippage, per one-way ticket. */
    costFeeBps: number;
...
export async function buildHistoryBrief(args: {
  ...
  return {
    coverage: {
      ...
      costFeeBps: data.costFeeBps,
      costSlippageBps: data.costSlippageBps,
      costFills: data.costFills,
