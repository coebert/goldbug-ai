...
describe("minor-unit rescaling precision", () => {
  ...
  it.each([
    ...
    const normalisedFee = check.normalised.fee;
    if (normalisedFee === undefined) {
      throw new Error("Expected normalised fee to be defined");
    }
    expectMoney(normalisedFee, pounds);
  });
