...
export const activateLive = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // ONE BROKER ACCOUNT PER PORTFOLIO — see broker-account-claim.ts. A DB
    // unique index backs this up; the pre-check just yields a readable message.
    await assertBrokerAccountUnclaimed(supabase as never, {
